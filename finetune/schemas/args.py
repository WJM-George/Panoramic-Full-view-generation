...
class Args(BaseModel):
    ...
    model_path: Path
    model_name: str
    model_type: Literal["i2v", "t2v", "v2v"]  # Added "v2v"
    training_type: Literal["lora", "sft"] = "lora"
    ...
    image_column: Path | None = None
    video_column: Path
    input_video_column: Path | None = None  # New field for V2V input videos
    ...
    train_resolution: Tuple[int, int, int]  # shape: (frames, height, width)
    condition_frames: int | None = None  # New field for number of input frames in V2V
    ...
    validation_input_videos: str | None  # New field for V2V validation input videos
    gen_fps: int = 15
    ...
    @field_validator("input_video_column")
    def validate_input_video_column(cls, v: str | None, info: ValidationInfo) -> str | None:
        values = info.data
        if values.get("model_type") == "v2v" and not v:
            raise ValueError("`input_video_column` must be specified for v2v model")
        return v
    ...
    @field_validator("validation_input_videos")
    def validate_validation_input_videos(cls, v: str | None, info: ValidationInfo) -> str | None:
        values = info.data
        if values.get("do_validation") and values.get("model_type") == "v2v" and not v:
            raise ValueError(
                "validation_input_videos must be specified when do_validation is True and model_type is v2v"
            )
        return v
    ...
    def parse_args(cls):
        ...
        parser.add_argument("--caption_column", type=str, required=True)
        parser.add_argument("--video_column", type=str, required=True)
        parser.add_argument("--input_video_column", type=str, default=None)  # New argument for V2V
        parser.add_argument("--gt_video_column", type=str, required=True) # new argument for v2v
        parser.add_argument("--train_resolution", type=str, required=True)
        parser.add_argument("--report_to", type=str, required=True)
        ...
        parser.add_argument("--validation_images", type=str, default=None)
        parser.add_argument("--validation_videos", type=str, default=None)
        parser.add_argument("--validation_input_videos", type=str, default=None)  # New argument for V2V
        parser.add_argument("--gen_fps", type=int, default=15)
        parser.add_argument("--condition_frames", type=int, default=None)  # New argument for V2V
